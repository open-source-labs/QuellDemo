...
const cacheHelpers_1 = require("./helpers/cacheHelpers");
/*
...
class QuellCache {
            ...
            if (operationType === "unQuellable") {
                ...
                if (redisValue != null) {
                    ...
                    (0, graphql_1.graphql)({ schema: this.schema, source: queryString })
                        .then((queryResult) => {
                        res.locals.queryResponse = queryResult;
                        (0, cacheHelpers_1.writeToCache)(queryString, queryResult, this.cacheExpiration);
                        return next();
                    })
                        .catch((error) => {
                        const err = {
                ...
                if (Object.keys(queryObject).length > 0) {
                    ...
                    (0, graphql_1.graphql)({ schema: this.schema, source: newQueryString })
                        ...
                        yield (0, cacheHelpers_1.normalizeForCache)(mergedResponse.data, this.queryMap, prototype, currName, this.cacheExpiration);
            ...
            for (const typeKey in prototype) {
                ...
                if (prototypeKeys.includes(typeKey)) {
                    ...
                    if (typeof subID === "string") {
                        ...
                        cacheID = (0, cacheHelpers_1.generateCacheID)(prototype[typeKey]);
                    }
                    let keyName;
                ...
                // if this iteration is a nested query (i.e. if typeKey is a field in the query)
                    ...
                    if (!(Object.keys(itemFromCache).length > 0) &&
                        ...
                        !typeKey.includes("__") &&
                        typeof prototype[typeKey] === "object") {
                        const cacheID = yield (0, cacheHelpers_1.generateCacheID)(prototype);
                        const cacheResponse = yield (0, redisHelpers_1.getFromRedis)(cacheID, this.redisCache);
                        if (cacheResponse)
    ...
    //       writeToCache(cacheID, fieldStore, this.cacheExpiration);
    //     }
    //   }
    // }
    // /**
            ...
            const removeFromFieldKeysList = (fieldKeysToRemove) => __awaiter(this, void 0, void 0, function* () {
                if (fieldsListKey) {
                    ...
                    if (cachedFieldKeysListRaw !== null &&
                        ...
                        (0, cacheHelpers_1.writeToCache)(fieldsListKey, cachedFieldKeysList, this.cacheExpiration);
                    }
                }
            });
            /**
                        ...
                        for (let i = 0; i < cachedFieldKeysList.length; i++) {
                            ...
                            if (fieldKeyValueRaw !== null && fieldKeyValueRaw !== undefined) {
                                ...
                                if (remove === true) {
                                    ...
                                    (0, cacheHelpers_1.deleteCacheById)(fieldKey.toLowerCase());
                                }
                            }
                        }
                    ...
                    cachedFieldKeysList.forEach((fieldKey) => __awaiter(this, void 0, void 0, function* () {
                        ...
                        if (fieldKeyValueRaw !== null && fieldKeyValueRaw !== undefined) {
                            ...
                            if (fieldsToUpdateBy.length > 0) {
                                (0, cacheHelpers_1.writeToCache)(fieldKey, updatedFieldKeyValue, this.cacheExpiration);
                            }
                        }
                    }));
                }
            ...
            if (redisKey) {
                ...
                if (mutationQueryObject.__id) {
                    ...
                    if (mutationName.substring(0, 3) === "del") {
                        ...
                        (0, cacheHelpers_1.deleteCacheById)(`${mutationType.toLowerCase()}--${mutationQueryObject.__id}`);
                        removeFromFieldKeysList([`${mutationType}--${dbRespId}`]);
                    }
                    else {
                        // Update mutation for single dbRespId
                        (0, cacheHelpers_1.writeToCache)(`${mutationType.toLowerCase()}--${mutationQueryObject.__id}`, dbRespData, this.cacheExpiration);
                    }
                }
                else {
                ...
                (0, cacheHelpers_1.writeToCache)(hypotheticalRedisKey, dbRespData, this.cacheExpiration);
            }
        });
    }
    // /**
