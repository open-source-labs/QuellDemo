...
const {QuellCache:QuellCache2} = require('../../src/quell.ts');

const schema2 = require('../../test-config/testSchema');
const redisPort2 = 6379;
const timeout = 100;

describe('server side tests for createQueryStr.js', () => {
  const Quell = new QuellCache2(schema2, redisPort2, timeout);
  //afterAll will run after all the tests in particular suite finished executing. 
  afterAll((done) => {
